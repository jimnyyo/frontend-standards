...
| 플러그인 | 언제 뜨나 | 내용 |
|---|---|---|
| `web-publishing-workflow` | 코드 쓰기 **전** | 공통기반→페이지구조→반응형→애니메이션 레이어 순서, 빌드 전 재사용 매핑 확인, 시안에 담기지 않는 움직임·인터랙션·상태 확인, 한 프로젝트 한 도구 |
...
`UNLICENSED` — 공개적으로 열람 가능하지만 사용권은 별도로 부여하지 않습니다.
